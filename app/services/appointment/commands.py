...
async def cancel_appointment_workflow(session: Session, appointment_id: int, client_phone_number: str):
    ...
    if appointment.client_id != client.id:
        return {"status": "error", "message": "Esta cita no te pertenece o no tienes permiso para cancelarla."}
    ...
    try:
        ...
        return {"status": "error", "message": "Error al cancelar la cita."}
async def create_appointment_workflow(session: Session, business_id: int, client_id: int, service_id: int, worker_id: int, start_time: datetime):
    ...
    if slot_exists:
        ...
        if not new_slots:
             return {
                "status": "error",
                "message": "El hueco solicitado acaba de ser ocupado y ya no quedan más citas libres para ese día.",
                "data": []
            }
    ...
    try:
        ...
        return {"status": "error", "message": "Error al guardar, el hueco fue ocupado."}
